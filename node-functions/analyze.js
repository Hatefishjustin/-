 ...
 * 文件路径: /node-functions/analyze.js
 * 访问路径: https://你的域名/analyze
 *
 * 作用：接收前端传来的"理想伴侣测评"原始作答记录，转发给阿里云百炼（DashScope）
 * 的通义千问模型做专业心理学角度的深度解读，再把结果返回给前端。
 *
 * 安全设计：
 * - API Key 只存在于 EdgeOne 控制台配置的环境变量（process.env.DASHSCOPE_API_KEY），
...
export const onRequestPost = async ({ request }) => {
  ...
  const ip = request.headers.get('x-forwarded-for') || request.headers.get('eo-client-ip') || 'unknown';
  const now = Date.now();
  ...
  if (!apiKey) {
    return jsonResponse({
      ...
      message: '服务器未配置阿里云百炼 API Key，请在 EdgeOne Pages 项目设置的环境变量中添加 DASHSCOPE_API_KEY。',
    }, 500);
  }
...
sections 数组应包含 4-5 个小节，覆盖：依恋模式深读、人格与沟通逻辑、爱语背后真正的心理需求、价值观内部的潜在张力、以及一条最重要的关系风险提示或建议。`;
  ...
  try {
    ...
    // 尝试解析为结构化 JSON；若模型偶尔仍返回了非纯JSON（如包裹了代码块），做一次兜底清洗
    let parsed;
    try {
